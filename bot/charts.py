...
def _fig_to_bytes(fig) -> io.BytesIO:
    ...
    buf = io.BytesIO()
    fig.savefig(
        buf, format="png", dpi=120, bbox_inches="tight", facecolor=BG, edgecolor="none"
    )
    plt.close(fig)
    buf.seek(0)
    return buf
...
def portfolio_donut(portfolio: dict, total_eval: float | None = None) -> io.BytesIO:
    ...
    colors = [
        TICKER_COLORS.get(h["ticker"], FLOOR_COLORS.get(h["floor"], "#888"))
        for h in holdings
    ]
    ...
    if total_eval is not None:
        ax.text(
            0, 0.1, "총 평가금액", ha="center", va="center", color=TEXT_DIM, fontsize=14
        )
        ax.text(
            0,
            -0.08,
...
def weight_comparison_bar(
    ...
    ax.set_title(
        "목표 비중 vs 현재 비중", color=TEXT, fontsize=18, fontweight="bold", pad=20
    )
...
# ── 3. 월별 분배금 추이 ─────────────────────────────────────────────
def dividend_trend_bar(
    monthly: dict[str, dict[str, int]], name_map: dict[str, str]
) -> io.BytesIO:
    """월별·종목별 분배금 스택 막대 차트.
    ...
    plt.setp(
        ax.get_xticklabels(),
        rotation=45 if len(months) > 6 else 0,
        ha="right",
        fontsize=11,
    )
